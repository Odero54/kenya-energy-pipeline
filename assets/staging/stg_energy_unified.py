...
conn.execute(
    "CREATE OR REPLACE TABLE staging.stg_energy_unified AS SELECT * FROM unified"
)
n = conn.execute("SELECT COUNT(*) FROM staging.stg_energy_unified").fetchone()[0]
conn.close()

logger.info(f"staging.stg_energy_unified: {n} rows")
